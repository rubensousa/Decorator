...
class GridSpanDelegate(private val context: Context) : DecorationDelegate() {
    ...
    override fun createLayoutManager(context: Context): RecyclerView.LayoutManager {
        val layoutManager = GridLayoutManager(
            context,
            DEFAULT_COLUMNS,
            decoration.getLayoutManager().orientation,
            decoration.getLayoutManager().reverseLayout
        )
    ...
    override fun setInverted(inverted: Boolean) {
        decoration.getLayoutManager().reverseLayout = inverted
    }

    override fun setOrientation(orientation: Int) {
        decoration.getLayoutManager().orientation = orientation
    }
